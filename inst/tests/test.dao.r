...
test_that("getKeyMap",{
  ...
  exp3<-getKeyMap(key=key)
  expect_more_than(nrow(exp3),16000)
})
...
test_that("getIndustryType",{
  ...
  exp2<-getIndustryType(qtid=c('000001.SZ','000002.SZ'),key=key)
  expect_that(unique(exp2$qtid),equals(c('000001.SZ','000002.SZ')))
  
  exp3<-getIndustryType(CompanyCode=6,key=key)
...
test_that("getFinancialIndex",{
  ...
  expect_that(unique(exp1$qtid),equals(c('000001.SZ','000002.SZ')))
  ...
  expect_that(unique(exp3$qtid),equals(c('000001.SZ','000002.SZ')))
